...
class PushoverNotifier:
    ...
    def _sendPushover(self, msg, title, userKey=None ):
        ...
        try:
            ...
            if not hasattr(e, 'code'):
                logger.log(u"Pushover notification failed." + ex(e))
                return False
            else:
                logger.log(u"Pushover notification failed. Error code: " + str(e.code))

            # HTTP status 404 if the provided email address isn't a Pushover user.
            if e.code == 404:
                logger.log(u"Username is wrong/not a pushover email. Pushover will send an email to it")
                return False
            ...
            # If you receive an HTTP status code of 400, it is because you failed to send the proper parameters
                ...
                logger.log(u"Wrong data sent to pushover")
                return False

        logger.log(u"Pushover notification successful.")
        return True
    ...
    def _notifyPushover(self, title, message, userKey=None ):
        ...
        if not sickbeard.USE_PUSHOVER:
            logger.log(u"Notification for Pushover not enabled, skipping this notification")
            return False
