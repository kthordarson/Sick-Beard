...
#        logger.log(u"DEBUG generic.py running getURL ... ")
        ...
        try:
            ...
            logger.log(u"DEBUG generic.py error" + ex(e))
            logger.log(u"DEBUG generic.py vesen med helpers.getURL "+url+" post_data "+" headers "+headers)

        if not data:
            logger.log(u"DEBUG generic.py Error loading ")
            return None
    ...
    def downloadResult(self, result):
        ...
        logger.log(u"Saving to " + file_name)
        ...
        if self.providerType == GenericProvider.TORRENT:
            ...
            if parser:
                ...
                if mime_type != 'application/x-bittorrent':
                    logger.log(u"Result is not a valid torrent file")
                    return False
...
#            quality = self.getQuality(item)
            ...
            if not show.air_by_date:
                ...
                if not parse_result.air_by_date:
                    logger.log(u"This is supposed to be an air-by-date search but the result "+title+" didn't parse as one, skipping it")
                    continue
                ...
                if len(sql_results) != 1:
                    logger.log(u"Tried to look up the date for the episode "+title+" but the database didn't give proper results, skipping it")
                    continue
            ...
            if len(epObj) == 1:
                ...
                logger.log(u"Separating multi-episode result to check for later - result contains episodes: " + str(parse_result.episode_numbers))
            elif len(epObj) == 0:
                epNum = SEASON_RESULT
