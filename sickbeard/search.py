...
def _downloadResult(result):
    ...
    if resProvider == None:
        logger.log(u"Invalid provider name - this is a coding error, report it please")
        return False
    ...
    # if it's an nzb data result
        ...
        try:
            ...
            logger.log(u"Error trying to save NZB to black hole: "+ex(e))
            newResult = False
        ...
        logger.log(u"Invalid provider type - this is a coding error, report it please")
        return False
...
def snatchEpisode(result, endStatus=SNATCHED):
    ...
    if result.resultType in ("nzb", "nzbdata"):
        if sickbeard.NZB_METHOD == "blackhole":
            ...
            logger.log(u"Unknown NZB action specified in config: " + sickbeard.NZB_METHOD)
            dlResult = False
    ...
    # TORRENTs can be sent to clients or saved to disk
        ...
        logger.log(u"Unknown result type, unable to download it")
        dlResult = False
...
def searchForNeededEpisodes():
    ...
    for curProvider in providers.sortedProviderList():
        ...
        for curEp in curFoundResults:
            ...
            if not bestResult:
                logger.log(u"All found results for "+curEp.prettyName()+" were rejected.")
                continue
    ...
    if not didSearch:
        logger.log(u"DEBUG search.py in failed in searchForNeededEpisodes  vesen ... ")
...
def pickBestResult(results, quality_list=None):
    ...
    logger.log(u"Picking the best result out of "+str([x.name for x in results]))
    ...
    if bestResult:
        logger.log(u"Picked "+bestResult.name+" as the best")
    else:
        logger.log(u"No result picked.")
...
def findEpisode(episode, manualSearch=False):
    ...
    for curProvider in providers.sortedProviderList():
        ...
        try:
            ...
            logger.log(u"DEBUG search.py findEpisode Error while searching "+curProvider.name+", skipping: "+ex(e))
            logger.log(traceback.format_exc())
            continue
    ...
    if not didSearch:
        logger.log(u"DEBUG search.py in findEpisode No NZB/Torrent providers found or enabled in the sickbeard config. Please check your settings.")
...
#            continue
#        except Exception, e:
#            logger.log(u"DEBUG search.py Error while searching "+curProvider.name+", skipping: "+ex(e))
#            logger.log(u"DEBUG search.py Error with curResults "+curResults)
            #logger.log(traceback.format_exc())
#            continue
    ...
    if bestSeasonNZB:
        ...
        logger.log(u"Episode list: "+str(allEps))
        ...
        if allWanted and bestSeasonNZB.quality == highest_quality_overall:
            ...
            logger.log(u"No eps from this season are wanted at this quality, ignoring the result of "+bestSeasonNZB.name)

        else:

            if bestSeasonNZB.provider.providerType == GenericProvider.NZB:
                logger.log(u"Breaking apart the NZB and adding the individual ones to our results")
    ...
    if MULTI_EP_RESULT in foundResults:
        for multiResult in foundResults[MULTI_EP_RESULT]:
            ...
            logger.log(u"Seeing if we want to bother with multi-episode result "+multiResult.name)
            ...
            logger.log(u"Single-ep check result is neededEps: "+str(neededEps)+", notNeededEps: "+str(notNeededEps))
            ...
            logger.log(u"Multi-ep check result is multiNeededEps: "+str(multiNeededEps)+", multiNotNeededEps: "+str(multiNotNeededEps))

            if not multiNeededEps:
                logger.log(u"All of these episodes were covered by another multi-episode nzbs, ignoring this multi-ep result")
                continue
            ...
            for epObj in multiResult.episodes:
                epNum = epObj.episode
                if epNum in foundResults:
                    logger.log(u"A needed multi-episode result overlaps with a single-episode result for ep #"+str(epNum)+", removing the single-episode results from the list")
