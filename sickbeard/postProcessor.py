...
class PostProcessor(object):
    ...
    def _checkForExistingFile(self, existing_file):
        ...
        if not existing_file:
            self._log(u"There is no existing file so there's no worries about replacing it")
        ...
        if ek.ek(os.path.isfile, existing_file):
            ...
            if ek.ek(os.path.getsize, existing_file) > ek.ek(os.path.getsize, self.file_path):
                self._log(u"File " + existing_file + " is larger than " + self.file_path)
                return PostProcessor.EXISTS_LARGER

            elif ek.ek(os.path.getsize, existing_file) == ek.ek(os.path.getsize, self.file_path):
                self._log(u"File " + existing_file + " is the same size as " + self.file_path)
                return PostProcessor.EXISTS_SAME
    
            else:
                self._log(u"File " + existing_file + " is smaller than " + self.file_path)
    ...
    def _delete(self, file_path, associated_files=False):
        ...
        if not file_list:
            self._log(u"There were no files associated with " + file_path + ", not deleting anything")
            return

        # delete the file and any other files which we want to delete
        for cur_file in file_list:
            self._log(u"Deleting file " + cur_file)
            if ek.ek(os.path.isfile, cur_file):
                ...
                if (not file_attribute & stat.S_IWRITE):
                    ...
                    try:
                        ...
                        self._log(u'Cannot change permissions of ' + cur_file)
    ...
    def _combined_file_operation (self, file_path, new_path, new_base_name, associated_files=False, action=None, subtitles=False):
        ...
        if not action:
            self._log(u"Must provide an action for the combined file operation")
            return
        ...
        if not file_list:
            self._log(u"There were no files associated with " + file_path + ", not moving anything")
            return
        ...
        for cur_file_path in file_list:
            ...
            if sickbeard.SUBTITLES_DIR and cur_extension in common.subtitleExtensions:
                ...
                if not dir_exists:
                    logger.log(u"Unable to create subtitles folder " + subs_new_path)
                else:
    ...
    def _move(self, file_path, new_path, new_base_name, associated_files=False, subtitles=False):
        ...
        def _int_move(cur_file_path, new_file_path):
            ...
            self._log(u"Moving file from " + cur_file_path + " to " + new_file_path)
            try:
                helpers.moveFile(cur_file_path, new_file_path)
                helpers.chmodAsParent(new_file_path)
            except (IOError, OSError), e:
                self._log("Unable to move file " + cur_file_path + " to " + new_file_path + ": " + str(e))
                raise e
    ...
    def _copy(self, file_path, new_path, new_base_name, associated_files=False, subtitles=False):
        ...
        def _int_copy (cur_file_path, new_file_path):
            ...
            self._log(u"Copying file from " + cur_file_path + " to " + new_file_path)
            try:
    ...
    def _hardlink(self, file_path, new_path, new_base_name, associated_files=False, subtitles=False):
        ...
        def _int_hard_link(cur_file_path, new_file_path):
            ...
            self._log(u"Hard linking file from " + cur_file_path + " to " + new_file_path)
            try:
    ...
    def _moveAndSymlink(self, file_path, new_path, new_base_name, associated_files=False, subtitles=False):
        ...
        def _int_move_and_sym_link(cur_file_path, new_file_path):
            ...
            self._log(u"Moving then symbolic linking file from " + cur_file_path + " to " + new_file_path)
            try:
    ...
    def _analyze_name(self, name, file=True):
        ...
        self._log("Parsed " + name + " into " + str(parse_result).decode('utf-8'))
        ...
        for cur_name in name_list:
            ...
            if scene_id:
                self._log(u"Scene exception lookup got tvdb id " + str(scene_id) + ", using that")
                _finalize(parse_result)
                return (scene_id, season, episodes)

        # see if we can find the name directly in the DB, if so use it
        for cur_name in name_list:
            self._log(u"Looking up " + cur_name +u" in the DB")
            db_result = helpers.searchDBForShow(cur_name)
            if db_result:
                self._log(u"Lookup successful, using tvdb id " + str(db_result[0]))
                _finalize(parse_result)
        ...
        for cur_name in name_list:
            try:
                ...
                try:
                    ...
                    self._log(u"Looking up name " + cur_name + u" in all languages on TVDB")
                    showObj = t[cur_name]
            ...
            self._log(u"Lookup successful, using tvdb id " + str(showObj["id"]))
            _finalize(parse_result)
        ...
        for cur_attempt in attempt_list:
            ...
            if season == -1 and tvdb_id and episodes:
                ...
                try:
                    ...
                    self._log(u"Got season " + str(season) + " episodes " + str(episodes))
                except tvdb_exceptions.tvdb_episodenotfound, e:
                    self._log(u"Unable to find episode with date " + str(episodes[0]) + u" for show " + str(tvdb_id) + u", skipping")
            ...
            # if there's no season then we can hopefully just use 1 automatically
                ...
                if int(numseasonsSQlResult[0][0]) == 1 and season == None:
                    self._log(u"Don't have a season number, but this show appears to only have 1 season, setting seasonnumber to 1...")
                    season = 1
    ...
    def _get_ep_obj(self, tvdb_id, season, episodes):
        ...
        for cur_episode in episodes:
            ...
            self._log(u"Retrieving episode object for " + str(season) + "x" + str(episode))
    
            # now that we've figured out which episode this file is just load it manually
            try:
                curEp = show_obj.getEpisode(season, episode)
            except exceptions.EpisodeNotFoundException, e:
                self._log(u"Unable to create episode: " + ex(e))
                raise exceptions.PostProcessingFailed()
    ...
    def _get_quality(self, ep_obj):
        ...
        if ep_obj.status in common.Quality.SNATCHED + common.Quality.SNATCHED_PROPER:
            ...
            if ep_quality != common.Quality.UNKNOWN:
                self._log(u"The old status had a quality in it, using that: " + common.Quality.qualityStrings[ep_quality])
                return ep_quality
        ...
        for cur_name in name_list:
            ...
            self._log(u"Looking up quality for name " + cur_name + u", got " + common.Quality.qualityStrings[ep_quality])
            
            # if we find a good one then use it
            if ep_quality != common.Quality.UNKNOWN:
                logger.log(cur_name + u" looks like it has quality " + common.Quality.qualityStrings[ep_quality] + ", using that")
                return ep_quality
        ...
        self._log(u"Guessing quality for name " + self.file_name+u", got " + common.Quality.qualityStrings[ep_quality])
        if ep_quality != common.Quality.UNKNOWN:
            logger.log(self.file_name + u" looks like it has quality " + common.Quality.qualityStrings[ep_quality] + ", using that")
            return ep_quality
    ...
    def _run_extra_scripts(self, ep_obj):
        ...
        for curScriptName in sickbeard.EXTRA_SCRIPTS:
            ...
            self._log(u"Absolute path to script: " + ek.ek(os.path.abspath, script_cmd[0]))
            try:
    ...
    def _is_priority(self, ep_obj, new_ep_quality):
        ...
        if self.in_history or ep_obj.status in common.Quality.SNATCHED + common.Quality.SNATCHED_PROPER:
            self._log(u"SB snatched this episode so I'm marking it as priority")
            return True
        ...
        if new_ep_quality > ep_obj and new_ep_quality != common.Quality.UNKNOWN:
            self._log(u"This was manually downloaded but it appears to be better quality than what we have so I'm marking it as priority")
            return True
    ...
    def process(self):
        ...
        if not tvdb_id or season == None or not episodes:
            self._log(u"Can't find show id from TVDB or season or episode, skipping")
            return False
        ...
        logger.log(u"Quality of the episode we're processing: " + str(new_ep_quality))
        ...
        if not priority_download:
            ...
            if existing_file_status in (PostProcessor.EXISTS_LARGER, PostProcessor.EXISTS_SAME):
                self._log(u"File exists and we are not going to replace it because it's not smaller, quitting post-processing")
                return False
            elif existing_file_status == PostProcessor.EXISTS_SMALLER:
                self._log(u"File exists and is smaller than the new file so I'm going to replace it")
        ...
        # if the file is priority then we're going to replace it even if it exists
            ...
            self._log(u"This download is marked a priority download so I'm going to replace an existing file if I find one")
        ...
        try:
            ...
            if self.process_method == "copy":
              ...
              logger.log(u"Unknown process method: " + sickbeard.PROCESS_METHOD)
