...
def getSeasonNZBs(name, urlData, season):
    ...
    try:
        ...
        logger.log(u"Unable to parse the XML of "+name+", not splitting it")
        return ({},'')
    ...
    if sceneNameMatch:
        ...
        logger.log(u"Unable to parse "+name+" into a scene name. If it's a valid one log a bug.")
        return ({},'')
...
def splitResult(result):
    ...
    if urlData is None:
        logger.log(u"Unable to load url "+result.url+", can't download season NZB")
        return False
    ...
    try:
        ...
        logger.log(u"Unable to parse the filename "+result.name+" into a valid episode")
        return False
    ...
    for newNZB in separateNZBs:
        ...
        try:
            ...
            logger.log(u"Unable to parse the filename "+newNZB+" into a valid episode")
            return False
        ...
        if (parse_result.season_number != None and parse_result.season_number != season) or (parse_result.season_number == None and season != 1):
            logger.log(u"Found "+newNZB+" inside "+result.name+" but it doesn't seem to belong to the same season, ignoring it")
            continue
        elif len(parse_result.episode_numbers) == 0:
            logger.log(u"Found "+newNZB+" inside "+result.name+" but it doesn't seem to be a valid episode NZB, ignoring it")
            continue

        wantEp = True
        for epNo in parse_result.episode_numbers:
            if not result.extraInfo[0].wantEpisode(season, epNo, result.quality):
                logger.log(u"Ignoring result "+newNZB+" because we don't want an episode that is "+Quality.qualityStrings[result.quality])
                wantEp = False
                break
        if not wantEp:
            continue
