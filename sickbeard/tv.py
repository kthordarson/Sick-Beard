...
class TVShow(object):
    ...
    def loadEpisodesFromDir (self):
        ...
        for mediaFile in mediaFiles:
            ...
            try:
                ...
                logger.log(u"Episode " + mediaFile + " returned an exception: " + ex(e))
                continue
            except exceptions.EpisodeDeletedException:
                logger.log(u"The episode deleted itself when I tried making an object for it")
            ...
            if curEpisode != None:
                if self.subtitles:
                    try:
                        ...
                        logger.log(str(self.tvdbid) + ": Could not refresh subtitles")
    ...
    def loadLatestFromTVRage(self):
        ...
        try:
            ...
            logger.log(u"Unable to add TVRage info: " + ex(e))
    ...
    def makeEpFromFile(self, file):
        ...
        try:
            ...
            logger.log(u"Unable to parse the filename " + file + " into a valid episode")
            return None

        if len(parse_result.episode_numbers) == 0 and not parse_result.air_by_date:
            logger.log("parse_result: " + str(parse_result))
            logger.log(u"No episode number found in " + file + ", ignoring it")
            return None
        ...
        if parse_result.air_by_date:
            try:
                ...
                logger.log(u"Unable to find episode with date " + str(parse_result.air_date) + " for show " + self.name + ", skipping")
                return None
        ...
        for curEpNum in episodes:
            ...
            if curEp == None:
                try:
                    ...
                    logger.log(str(self.tvdbid) + u": Unable to figure out what this file is, skipping")
                    continue

            else:
                # if there is a new file associated with this ep then re-check the quality
                if curEp.location and ek.ek(os.path.normpath, curEp.location) != ek.ek(os.path.normpath, file):
                    logger.log(u"The old episode had a different file associated with it, I will re-check the quality based on the new filename " + file)
...
#        self.saveToDB()
    ...
    def loadNFO(self):
        ...
        try:
            ...
            try:
                ...
                logger.log(u"Failed to rename your tvshow.nfo file - you need to delete it or fix it: " + ex(e))
    ...
    def downloadSubtitles(self, force=False):
        ...
        try:
            ...
            logger.log("Error occurred when downloading subtitles: " + traceback.format_exc())
            return
    ...
    def wantEpisode(self, season, episode, quality, manualSearch=False):
        ...
        if quality in anyQualities + bestQualities:
            if epStatus in (WANTED, UNAIRED, SKIPPED):
                logger.log(u"Ep is wanted/unaired/skipped, definitely get it")
                return True
            elif manualSearch:
                logger.log(u"Usually I would ignore this ep but because you forced the search I'm overriding the default and allowing the quality")
                return True
            else:
                logger.log(u"This quality looks like something we might want but I don't know for sure yet")
        ...
        if curStatus in Quality.DOWNLOADED + Quality.SNATCHED + Quality.SNATCHED_PROPER and quality in bestQualities and quality > curQuality:
            logger.log(u"We already have this ep but the new one is better quality, saying yes")
            return True

        logger.log(u"None of the conditions were met so I'm just saying no")
        return False
...
class TVEpisode(object):
        ...
        if not sqlResult:
            ...
            if ek.ek(os.path.isfile, self.location):
                try:
                    ...
                    logger.log(str(self.show.tvdbid) + u": There was an error loading the NFO for episode " + str(season) + "x" + str(episode))
                    pass
        ...
        if not ek.ek(os.path.isfile, self.location):
            ...
            # if there's no airdate then set it to skipped (and respect ignored)
                ...
                if self.status == IGNORED:
                    logger.log(u"Episode has no air date, but it's already marked as ignored")
                else:
                    logger.log(u"Episode has no air date, automatically marking it skipped")
                    self.status = SKIPPED
            # if we don't have the file and the airdate is in the past
                ...
                # if we somehow are still UNKNOWN then just skip it
                    ...
                    logger.log(u"Not touching status because we have no ep file, the airdate is in the past, and the status is "+str(self.status))
        ...
        if self.location != "":
            ...
            if ek.ek(os.path.isfile, nfoFile):
                try:
                    ...
                    logger.log(u"Error loading the NFO, backing up the NFO and skipping for now: " + ex(e)) #TODO: figure out what's wrong and fix it
                    try:
                        ek.ek(os.rename, nfoFile, nfoFile + ".old")
                    except Exception, e:
                        logger.log(u"Failed to rename your episode's NFO file - you need to delete it or fix it: " + ex(e))
    ...
    def deleteEpisode(self):
        ...
        if self.show.getEpisode(self.season, self.episode, noCreate=True) == self:
            logger.log(u"Removing myself from my show's list")
            del self.show.episodes[self.season][self.episode]

        # delete myself from the DB
        logger.log(u"Deleting myself from the database")
        myDB = db.DBConnection()
    ...
    def rename(self):
        ...
        for cur_related_file in related_files:
            ...
            if cur_result == False:
                logger.log(str(self.tvdbid) + u": Unable to rename file " + cur_related_file)

        for cur_related_sub in related_subs:
            cur_result = helpers.rename_ep_file(cur_related_sub, absolute_proper_subs_path, absolute_current_path_no_ext_length)
            if cur_result == False:
                logger.log(str(self.tvdbid) + u": Unable to rename file " + cur_related_sub)
