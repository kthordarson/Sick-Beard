...
class ConfigPostProcessing:
    ...
    def isRarSupported(self):
        ...
        try:
            ...
            logger.log(u'Rar Not Supported: ' + ex(e))
            return 'not supported'
...
class NewHomeAddShows:
    ...
    def searchTVDBForShowName(self, name, lang="en"):
        ...
        logger.log(u"Trying to find Show on thetvdb.com with: " + nameUTF8.decode('utf-8'))
        ...
        for searchTerm in keywords:
            ...
            logger.log(u"Searching for Show with searchterm: \'" + searchTerm.decode('utf-8') + u"\' on URL " + finalURL)
            urlData = helpers.getURL(finalURL)

            if urlData is None:
                # When urlData is None, trouble connecting to TVDB, don't try the rest of the keywords
                logger.log(u"Unable to get URL: " + finalURL)
                break
            else:
                try:
                    ...
                    logger.log(u"Unable to parse XML for some reason: " + ex(e) + " from XML: " + finalURL)
                    series = ''
...
class Home:
    ...
    def setStatus(self, show=None, eps=None, status=None, direct=False):
        ...
        if eps != None:
            ...
            for curEp in eps.split('|'):
                ...
                logger.log(u"Attempting to set status on episode "+curEp+" to "+status)
                ...
                with epObj.lock:
                    ...
                    if epObj.status == UNAIRED:
                        logger.log(u"Refusing to change status of "+curEp+" because it is UNAIRED")
                        continue
                    ...
                    if int(status) in Quality.DOWNLOADED and epObj.status not in Quality.SNATCHED + Quality.SNATCHED_PROPER + Quality.DOWNLOADED + [IGNORED] and not ek.ek(os.path.isfile, epObj.location):
                        logger.log(u"Refusing to change status of "+curEp+" to DOWNLOADED because it's not SNATCHED/DOWNLOADED")
                        continue

                    if int(status) == FAILED and epObj.status not in Quality.SNATCHED + Quality.SNATCHED_PROPER + Quality.DOWNLOADED:
                        logger.log(u"Refusing to change status of "+curEp+" to FAILED because it's not SNATCHED/DOWNLOADED")
                        continue
    ...
    def doRename(self, show=None, eps=None):
        ...
        for curEp in eps.split('|'):
            ...
            if not ep_result:
                logger.log(u"Unable to find an episode for " + curEp + ", skipping")
                continue
